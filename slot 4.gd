...
func _physics_process(delta):
	$AnimatedSprite.play(CardLogic.hand[3])
	if Input.is_action_just_pressed("hand_slot_4") and $AnimatedSprite.scale <= Vector2(0.3744,0.384):
		$AnimatedSprite.scale *=1.2
		CardLogic.slot_4_active = true
		print("scale")
	elif Input.is_action_just_pressed("hand_reset"):
		CardLogic.slot_4_active = false
